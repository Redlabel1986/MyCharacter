...
const bodySchema = z.object({
  ...
  kind: z.enum(['weapon', 'armor', 'consumable']).default('weapon'),
  price: z.string().max(40).default(''),
  priceGold: z.number().int().min(0).max(99999).default(0),
  priceSilver: z.number().int().min(0).max(99999).default(0),
  priceCopper: z.number().int().min(0).max(99999).default(0),
...
export default defineEventHandler(async (event) => {
  ...
  const [item] = await db
    ...
    .values({
      ...
      priceGold: body.priceGold,
      priceSilver: body.priceSilver,
      priceCopper: body.priceCopper,
      damage: body.damage.trim(),
      armor: body.armor ?? null,
      healAmount: body.healAmount ?? null,
      manaAmount: body.manaAmount ?? null,
