...
/**
 * Battle-Map-Detailseite — komplette Spiel-Konsole.
 * Funktionen:
 *  - Map-Anzeige mit Quadrat- oder Hex-Raster, Pan + Zoom, Mittig-Layout
 *  - Token-System: Drag, Snap-to-Grid, Bild aus Charakter oder Custom-Upload,
 *    HP-Editor mit Wunden-Overlay, Status-Conditions mit Icons
 *  - Zeichen-Werkzeuge (Stift, Radierer, Farben, Pinsel, Undo, Clear-All)
 *  - Pings (Alt+Klick)
 *  - Initiative-Tracker (DM)
 *  - Audio-Panel (YouTube/Spotify-Link + Upload-MP3, mit Lautstärke + Sync)
 *  - Mini-Charsheet pro Token mit Tabs (HP, Skill-Roll, Inventar)
 *  - Chat-Sidebar (resizable) mit Whisper + /roll
 *  - Map-Switcher fuer DM, Auto-Redirect der Spieler bei aktive-Karten-Wechsel
 */
...
// Pre-deklarationen wegen TDZ in fetchMap
    ...
    if (protectedIds.size === 0) {
      ...
      tokens.value = res.tokens.map((t) =>
        protectedIds.has(t.id)
          ? tokens.value.find((x) => x.id === t.id) ?? t
          : t,
      )
    }
  } catch (e: unknown) {
...
const gridSvg = computed(() => {
  ...
  if (map.value.gridType === 'square') {
    return `<svg xmlns="http://www.w3.org/2000/svg" width="${imgW.value}" height="${imgH.value}" viewBox="0 0 ${imgW.value} ${imgH.value}"><defs><pattern id="grid" width="${g}" height="${g}" patternUnits="userSpaceOnUse"><path d="M ${g} 0 L 0 0 0 ${g}" fill="none" stroke="${color}" stroke-width="1"/></pattern></defs><rect width="100%" height="100%" fill="url(#grid)"/></svg>`
  }
  const s = g / 2
  const colStep = 1.5 * s
...
const snap = (x: number, y: number) => {
  ...
  const yOff = c % 2 ? rowStep / 2 : 0
  const r = Math.round((y - yOff) / rowStep)
  return { x: c * colStep, y: r * rowStep + yOff }
}
...
const activeMusicEmbedUrl = computed(() => {
  ...
  if (!s?.isPlaying || !t || t.provider !== 'spotify') return null
  try {
...
const destroyYtPlayer = () => {
  try { ytPlayer.value?.destroy() } catch { /* ignore */ }
  ytPlayer.value = null
}
...
watch(audioVolume, (v) => {
  if (!ytPlayer.value || audioMuted.value) return
  try { ytPlayer.value.setVolume(Math.max(0, Math.min(100, Math.round(v * 100)))) } catch {/**/}
})
watch(audioMuted, (m) => {
...
watch(
  ...
  (sfxAt) => {
    ...
    if (t.provider === 'upload') {
      ...
      try { sfxEmbedUrl.value = audioEmbedUrl(t.audioUrl, { autoplay: true }) }
      catch { sfxEmbedUrl.value = null }
...
const dmPlayMusic = async (trackId: number) => {
  await $fetch(`/api/groups/${groupId}/audio/state`, { method: 'PUT', body: { action: 'play', trackId } })
  await fetchMap()
}
const dmStopMusic = async () => {
  await $fetch(`/api/groups/${groupId}/audio/state`, { method: 'PUT', body: { action: 'stop' } })
  await fetchMap()
}
const dmTriggerSfx = async (trackId: number) => {
  await $fetch(`/api/groups/${groupId}/audio/state`, { method: 'PUT', body: { action: 'sfx', trackId } })
  await fetchMap()
}
...
const audioAddProvider = computed(() => {
  ...
  try { return parseAudioUrl(audioAddUrl.value).provider } catch { return null }
})
const addAudioTrack = async () => {
  ...
  try {
    await $fetch(`/api/groups/${groupId}/audio/tracks`, {
      method: 'POST',
      body: { name: audioAddName.value.trim(), kind: audioAddKind.value, url: audioAddUrl.value.trim() },
    })
    audioAddUrl.value = ''
    audioAddName.value = ''
    await fetchAudioTracks()
  } catch (e: unknown) {
    audioAddError.value = (e as { statusMessage?: string }).statusMessage ?? 'Konnte Track nicht hinzufügen.'
  } finally {
...
const uploadAudio = async () => {
  ...
  try {
    ...
    audioUploadError.value = (e as { statusMessage?: string }).statusMessage ?? 'Upload fehlgeschlagen.'
  } finally {
...
const saveInitiative = async (state: InitiativeState | null) => {
  await $fetch(`/api/groups/${groupId}/initiative`, { method: 'PUT', body: state })
...
const initStartFromTokens = async () => {
  ...
  await saveInitiative({ active: true, round: 1, currentIndex: 0, entries })
}
const initAddManual = async () => {
  const name = prompt('Name?')?.trim()
  if (!name) return
  const initiative = Number.parseInt(prompt('Initiative?', '10') ?? '10', 10) || 10
  const entries = [
    ...(initiativeState.value?.entries ?? []),
    { id: `manual-${Date.now()}`, name, initiative, hasActed: false },
  ]
...
const initSetInitiative = async (id: string, value: number) => {
  if (!initiativeState.value) return
  const entries = initiativeState.value.entries.map((e) => (e.id === id ? { ...e, initiative: value } : e))
  await saveInitiative({ ...initiativeState.value, entries })
}
const initNextTurn = async () => {
  ...
  const currentIdx = sorted.findIndex((e) => e.id === initCurrentEntryId.value)
  const nextIdx = currentIdx + 1
  if (nextIdx >= sorted.length) {
    const entries = s.entries.map((e) => ({ ...e, hasActed: false }))
    await saveInitiative({ ...s, entries, round: s.round + 1, currentIndex: 0 })
  } else {
    const entries = s.entries.map((e) =>
      e.id === sorted[currentIdx]?.id ? { ...e, hasActed: true } : e,
    )
...
const initPrevTurn = async () => {
  ...
  if (prevIdx < 0) {
    if (s.round > 1) {
      ...
      await saveInitiative({ ...s, entries, round: s.round - 1, currentIndex: Math.max(0, sorted.length - 1) })
    }
    return
  }
  const prevId = sorted[prevIdx]?.id
  const entries = s.entries.map((e) => (e.id === prevId ? { ...e, hasActed: false } : e))
...
const DRAW_COLORS = ['#ef4444', '#f59e0b', '#eab308', '#10b981', '#06b6d4', '#3b82f6', '#8b5cf6', '#ec4899', '#000000', '#ffffff']
...
const deleteDrawing = async (d: Drawing) => {
  ...
  try {
    await $fetch(`/api/groups/${groupId}/maps/${mapId}/drawings/${d.id}`, { method: 'DELETE' })
  } catch {
    await fetchMap()
  }
}
...
const clearAllDrawings = async () => {
  if (!isDm.value) return
  if (!confirm('Alle Zeichnungen auf dieser Karte löschen?')) return
  drawings.value = []
  try {
...
const onStagePointerDown = (e: PointerEvent) => {
  ...
  if (toolMode.value === 'draw') startDrawing(e)
  else if (toolMode.value === 'select') startPan(e)
}
const onStagePointerMove = (e: PointerEvent) => {
  if (currentStroke.value) { continueDrawing(e); return }
  if (pan.active) { updatePan(e); return }
  onPointerMove(e)
}
const onStagePointerUp = (e: PointerEvent) => {
  if (currentStroke.value) { endDrawing(); return }
  if (pan.active) { endPan(); return }
  onPointerUp(e)
}
...
const newToken = ref({ characterId: 0, name: '', description: '', size: 1, hidden: false, imageFile: null as File | null })
const openAdd = async () => {
  showAddModal.value = true
  newToken.value = { characterId: 0, name: '', description: '', size: 1, hidden: false, imageFile: null }
...
const addToken = async () => {
  ...
  try {
    ...
    if (newToken.value.imageFile && created.token) {
      ...
      await $fetch(`/api/groups/${groupId}/maps/${mapId}/tokens/${created.token.id}/image`, { method: 'POST', body: fd })
    }
    showAddModal.value = false
    await fetchMap()
  } catch (e: unknown) {
    addTokenError.value = (e as { statusMessage?: string }).statusMessage ?? 'Token konnte nicht angelegt werden.'
  } finally {
    addingToken.value = false
  }
}
...
const saveEdit = async () => {
  ...
  if (editImageFile.value) {
    ...
    await $fetch(`/api/groups/${groupId}/maps/${mapId}/tokens/${t.id}/image`, { method: 'POST', body: fd })
  }
...
const customStatusText = computed<string>({
  ...
  set: (val: string) => {
    if (!editing.value) return
    const parsed = parseStatusText(editing.value.statusText ?? '')
    const newCustom = val.split(',').map((s) => s.trim()).filter(Boolean)
    editing.value.statusText = buildStatusText(parsed.conditions.map((c) => c.id), newCustom)
  },
})
...
const saveSettings = async () => {
  ...
  try {
    await $fetch(`/api/groups/${groupId}/maps/${mapId}`, { method: 'PUT', body: settingsDraft.value })
    await fetchMap()
  } finally {
...
const setThisActive = async () => {
  await $fetch(`/api/groups/${groupId}/active-map`, { method: 'PUT', body: { mapId } })
  await fetchMap()
}
const clearActive = async () => {
  await $fetch(`/api/groups/${groupId}/active-map`, { method: 'PUT', body: { mapId: null } })
  await fetchMap()
}
const switchToMap = async (newMapId: number, makeActive: boolean) => {
  if (makeActive) {
    await $fetch(`/api/groups/${groupId}/active-map`, { method: 'PUT', body: { mapId: newMapId } })
  }
...
watch(chatWidth, (v) => {
  if (typeof window !== 'undefined') localStorage.setItem('battlemap.chatWidth', String(v))
})
...
const endResize = () => { resizing.value = false }

// Mini-Charsheet: meine Tokens auf dieser Map
const myTokensOnMap = computed(() => {
  if (!user.value) return []
  return tokens.value
    .filter((t) => t.ownerUserId === user.value!.id)
    .slice()
    .sort((a, b) => {
      const aHas = a.characterId !== null
      const bHas = b.characterId !== null
  ...
  <div v-if="map" class="flex flex-col lg:flex-row gap-3">
    ...
    <div class="space-y-3 min-w-0 flex-1">
      ...
      <div class="flex items-center gap-3 flex-wrap">
        <NuxtLink :to="`/groups/${groupId}/battle`" class="text-sm text-[var(--color-accent)] hover:underline">
          ← Karten
        </NuxtLink>
        ...
        <div class="flex items-center gap-2 flex-wrap">
          ...
          <UButton v-if="isDm" size="sm" variant="outline" icon="i-lucide-settings" @click="settingsOpen = !settingsOpen">
            Karte
          </UButton>
          <UButton color="primary" icon="i-lucide-plus" size="sm" @click="openAdd">Token</UButton>
          <UButton class="lg:hidden" size="sm" variant="ghost"
      ...
      <div v-if="isDm && settingsOpen" class="parchment-card p-4 space-y-4">
        <section class="space-y-2">
          <div class="flex items-baseline justify-between gap-3">
            ...
            <div class="flex items-center gap-2">
              <UButton v-if="activeMapId !== mapId" size="xs" color="primary" icon="i-lucide-play" @click="setThisActive">
                Diese aktiv setzen
              </UButton>
              <span v-else class="text-[10px] uppercase tracking-widest font-semibold bg-[var(--color-accent)] text-white px-2 py-0.5 rounded">
                Aktiv
              </span>
              <UButton v-if="activeMapId !== null" size="xs" variant="ghost" @click="clearActive">
                Aktiv entfernen
              </UButton>
            </div>
          </div>
          <p class="text-[11px] text-ink-300">Spieler werden automatisch auf die aktive Karte geleitet.</p>
        </section>
        ...
        <section v-if="allMapsForSwitcher.length" class="space-y-2">
          ...
          <div class="grid sm:grid-cols-3 lg:grid-cols-4 gap-2">
            <button v-for="m in allMapsForSwitcher" :key="m.id" type="button"
              ...
              :disabled="m.id === mapId" @click="switchToMap(m.id, true)">
              <div class="aspect-video bg-black/10 rounded overflow-hidden flex items-center justify-center mb-1 relative">
                <img :src="`/api/groups/${groupId}/maps/${m.id}/image`" class="w-full h-full object-cover" loading="lazy" :alt="m.name">
                <span v-if="m.id === activeMapId" class="absolute top-1 left-1 text-[9px] uppercase tracking-widest font-semibold bg-[var(--color-accent)] text-white px-1 rounded">
                  Aktiv
                </span>
              </div>
        ...
        <section>
          ...
          <div class="grid sm:grid-cols-12 gap-3">
            <UFormField label="Name" class="sm:col-span-4"><UInput v-model="settingsDraft.name" /></UFormField>
            <UFormField label="Raster" class="sm:col-span-2">
              <USelect v-model="settingsDraft.gridType"
                :items="[{label:'Quadrat',value:'square'},{label:'Hex',value:'hex'}]" value-key="value" />
            </UFormField>
            <UFormField label="Größe (px)" class="sm:col-span-2">
          ...
          <UButton class="mt-2" color="primary" icon="i-lucide-save" :loading="savingSettings" @click="saveSettings">Speichern</UButton>
        </section>
      </div>
      ...
      <div class="parchment-card p-2 flex items-center gap-2 flex-wrap">
        <div class="flex items-center gap-1">
          <UButton size="xs" :variant="toolMode === 'select' ? 'solid' : 'outline'" :color="toolMode === 'select' ? 'primary' : 'neutral'" icon="i-lucide-mouse-pointer-2" @click="toolMode = 'select'">
            Auswählen
          </UButton>
          <UButton size="xs" :variant="toolMode === 'draw' ? 'solid' : 'outline'" :color="toolMode === 'draw' ? 'primary' : 'neutral'" icon="i-lucide-pencil" @click="toolMode = 'draw'">
            Zeichnen
          </UButton>
          <UButton size="xs" :variant="toolMode === 'erase' ? 'solid' : 'outline'" :color="toolMode === 'erase' ? 'primary' : 'neutral'" icon="i-lucide-eraser" @click="toolMode = 'erase'">
            Radieren
          </UButton>
        </div>
        <div v-if="toolMode === 'draw'" class="flex items-center gap-2 flex-wrap pl-3 border-l border-parchment-700/30">
          <div class="flex items-center gap-1">
            <button v-for="c in DRAW_COLORS" :key="c" type="button"
              ...
              :style="{ background: c }" @click="drawColor = c" />
          </div>
        ...
        <div class="ml-auto flex items-center gap-2 flex-wrap">
          <UButton size="xs" variant="ghost" icon="i-lucide-undo-2" @click="undoLastMine">Rückgängig</UButton>
          <UButton v-if="isDm" size="xs" variant="ghost" color="error" icon="i-lucide-trash-2" @click="clearAllDrawings">
      ...
      <div class="parchment-card p-2">
        <div ref="stageWrapperEl" class="overflow-auto bg-black/5 rounded flex"
          style="max-height: 78vh; place-content: safe center; place-items: safe center;">
          <div class="relative flex-none" :style="{ width: (imgW * zoom) + 'px', height: (imgH * zoom) + 'px' }">
            <div ref="stageEl" class="absolute inset-0 origin-top-left"
              :style="{
                width: imgW + 'px', height: imgH + 'px', transform: `scale(${zoom})`,
                backgroundImage: gridSvgUrl, backgroundSize: 'contain', backgroundRepeat: 'no-repeat',
                cursor: stageCursor, touchAction: toolMode === 'draw' ? 'none' : 'auto',
              }"
              @pointerdown="onStagePointerDown" @pointermove="onStagePointerMove"
              @pointerup="onStagePointerUp" @pointercancel="onStagePointerUp">
              <img :src="`/api/groups/${groupId}/maps/${mapId}/image`" :alt="map.name"
              ...
              <!-- Zeichnungen -->
              <svg v-if="imgW && imgH" class="absolute inset-0 pointer-events-none"
                :width="imgW" :height="imgH" :viewBox="`0 0 ${imgW} ${imgH}`">
                <path v-for="d in drawings" :key="d.id" :d="pointsToPath(d.points)"
                  fill="none" :stroke="d.color" :stroke-width="d.strokeWidth"
                  stroke-linecap="round" stroke-linejoin="round"
                  :class="[toolMode === 'erase' && canDeleteDrawing(d) ? 'cursor-pointer hover:opacity-60' : '']"
                  :style="{ pointerEvents: toolMode === 'erase' && canDeleteDrawing(d) ? 'stroke' : 'none' }"
                  @click="toolMode === 'erase' && canDeleteDrawing(d) && deleteDrawing(d)" />
                <path v-if="currentStroke && currentStroke.points.length > 1"
                  :d="pointsToPath(currentStroke.points)"
                  fill="none" :stroke="currentStroke.color" :stroke-width="currentStroke.strokeWidth"
                ...
                <g v-for="p in visiblePings" :key="`ping-${p.id}`" class="pointer-events-none">
                  <circle :cx="p.x" :cy="p.y" :r="map.gridSize * 0.4" fill="none"
                    :stroke="p.color" stroke-width="3" opacity="0.9">
                    <animate attributeName="r" :from="map.gridSize * 0.4" :to="map.gridSize * 0.9" dur="1s" repeatCount="indefinite" />
              ...
              <!-- Tokens -->
              <div v-for="t in tokens" :key="t.id" :data-token-id="t.id"
                ...
                :class="[
                  toolMode !== 'select' ? 'cursor-default' : (canMoveToken(t) ? 'cursor-move' : 'cursor-pointer'),
                ...
                <img v-if="tokenImageSrc(t)" :src="tokenImageSrc(t) ?? ''" :alt="t.name"
                  class="w-full h-full object-cover rounded-full pointer-events-none" draggable="false">
                <span v-else class="text-center px-1 leading-tight pointer-events-none">{{ t.name.slice(0, 8) }}</span>
                <!-- Verwundungs-Schleier -->
                <div v-if="tokenDamageOverlay(t)" class="absolute inset-0 rounded-full pointer-events-none"
                  :style="{
                ...
                <div v-if="t.hp !== null && t.hp <= 0 && t.hpMax"
                ...
                <div class="absolute -top-2 left-1/2 -translate-x-1/2 flex gap-0.5 pointer-events-none">
                  <div v-for="c in tokenConditions(t).slice(0, 6)" :key="c.id"
                    class="w-5 h-5 rounded-full border-2 flex items-center justify-center shadow"
                    :style="condStyle(c)" :title="c.label + ' — ' + c.hint">
                    <UIcon :name="c.icon" class="size-3.5" />
                  </div>
                  <div v-if="tokenConditions(t).length > 6"
                ...
                <div v-if="tokenCustomLabels(t).length"
      ...
      <div v-if="initiativeState || isDm" class="parchment-card p-3 space-y-2">
        <div class="flex items-center justify-between gap-2 flex-wrap">
          <h2 class="font-serif text-lg flex items-center gap-2">
            <UIcon name="i-lucide-swords" /> Initiative
          </h2>
          <div v-if="initiativeState" class="flex items-center gap-1">
            <UButton v-if="isDm" size="xs" variant="outline" icon="i-lucide-minus"
              :disabled="initiativeState.round <= 1" @click="initSetRound(initiativeState.round - 1)" />
            <div class="px-3 py-1 rounded font-serif font-semibold text-base bg-[var(--color-accent-soft)] border border-[var(--color-accent)]/40 flex items-center gap-2">
              <span class="text-[10px] uppercase tracking-widest text-[var(--color-accent)]">Runde</span>
              <input v-if="isDm" type="number" min="1" max="9999" :value="initiativeState.round"
            ...
            <UButton v-if="isDm" size="xs" variant="outline" icon="i-lucide-plus" @click="initSetRound(initiativeState.round + 1)" />
            <UButton v-if="isDm" size="xs" variant="ghost" icon="i-lucide-rotate-ccw" @click="initResetRound" />
          </div>
          <div v-if="isDm" class="flex gap-2 flex-wrap">
            <UButton v-if="!initiativeState" size="xs" color="primary" icon="i-lucide-play" @click="initStartFromTokens">
              Mit Tokens starten
            </UButton>
            <UButton size="xs" variant="outline" icon="i-lucide-plus" @click="initAddManual">Manuell</UButton>
            <UButton v-if="initiativeState" size="xs" variant="outline" icon="i-lucide-skip-back"
              :disabled="initiativeState.currentIndex === 0 && initiativeState.round === 1" @click="initPrevTurn">
              Zurück
            </UButton>
            <UButton v-if="initiativeState" size="xs" color="primary" icon="i-lucide-skip-forward" @click="initNextTurn">
              Nächster Zug
            </UButton>
            <UButton v-if="initiativeState" size="xs" variant="ghost" color="error" @click="initEnd">Beenden</UButton>
          </div>
        </div>
        <ol v-if="initEntries.length" class="space-y-1">
          <li v-for="(e, idx) in initEntries" :key="e.id"
            ...
            <img v-if="e.imageUrl" :src="e.imageUrl" :alt="e.name"
            ...
            <input v-if="isDm" type="number" :value="e.initiative"
            ...
            <UButton v-if="isDm" size="xs" variant="ghost" color="error" icon="i-lucide-x" @click="initRemoveEntry(e.id)" />
          </li>
        </ol>
      </div>

      <!-- Mini-Charsheet (mit Tabs für mehrere Tokens) -->
      <MiniCharSheet :group-id="groupId" :map-id="mapId" :tokens="myTokensOnMap" @token-updated="fetchMap" />

      <!-- Audio-Panel -->
      <div class="parchment-card p-3 space-y-2">
        <div class="flex items-center justify-between gap-2 flex-wrap">
          <h2 class="font-serif text-lg flex items-center gap-2">
            <UIcon name="i-lucide-music" /> Audio
            <span v-if="currentTrack" class="text-xs text-ink-400 font-normal italic">▶ {{ currentTrack.name }}</span>
          </h2>
          <div class="flex items-center gap-2">
            <UButton size="xs" variant="ghost"
              :icon="audioMuted ? 'i-lucide-volume-x' : 'i-lucide-volume-2'"
              @click="audioMuted = !audioMuted" />
            <input v-model.number="audioVolume" type="range" min="0" max="1" step="0.05" class="w-20" :disabled="audioMuted">
            <UButton v-if="isDm && audioState?.isPlaying" size="xs" variant="outline" icon="i-lucide-square" @click="dmStopMusic">
              Stopp
            </UButton>
            <UButton size="xs" variant="ghost" :icon="audioCollapsed ? 'i-lucide-chevron-down' : 'i-lucide-chevron-up'" @click="audioCollapsed = !audioCollapsed" />
          </div>
        </div>

        <!-- YouTube-Player -->
        <div v-show="activeYouTubeTrack" class="rounded overflow-hidden bg-black/5" style="height: 180px">
          <div ref="ytPlayerContainer" class="w-full h-full" />
        </div>
        <!-- Spotify-Embed -->
        <div v-if="activeMusicEmbedUrl" class="rounded overflow-hidden bg-black/5">
          <iframe :src="activeMusicEmbedUrl" class="w-full" style="height: 152px"
            allow="autoplay; encrypted-media; fullscreen" allowfullscreen loading="lazy" frameborder="0" title="Spotify-Player" />
        ...
        <!-- Upload-Audio -->
        <audio v-if="activeMusicStreamUrl" ref="audioPlayerEl" :src="activeMusicStreamUrl" :muted="audioMuted" loop controls class="w-full" />
        <div v-else-if="audioState?.isPlaying && !activeMusicEmbedUrl && !activeYouTubeTrack" class="text-xs text-ink-400 italic">
          Track wird vorbereitet …
        </div>

        <!-- Großer Lautstärke-Slider -->
        <div v-if="audioState?.isPlaying && currentTrack" class="flex items-center gap-3 mt-2 px-3 py-2 rounded bg-white/70 border border-parchment-700/30">
          <UButton size="sm" variant="ghost"
            ...
            :disabled="currentTrack.provider === 'spotify'" @click="audioMuted = !audioMuted" />
          <input v-model.number="audioVolume" type="range" min="0" max="1" step="0.01"
          ...
          <span class="text-xs tabular-nums w-10 text-right text-ink-400">
            {{ currentTrack.provider === 'spotify' ? 'SP' : audioMuted ? '—' : Math.round(audioVolume * 100) + '%' }}
          </span>
        </div>

        <!-- SFX (versteckt) -->
        <iframe v-if="sfxEmbedUrl" :src="sfxEmbedUrl" class="hidden" allow="autoplay; encrypted-media" frameborder="0" />
        <audio v-if="sfxStreamUrl" ref="sfxAudioEl" :src="sfxStreamUrl" :muted="audioMuted" autoplay class="hidden" />
        ...
        <div v-if="!audioCollapsed && isDm">
          ...
          <div class="grid sm:grid-cols-12 gap-2 items-end mb-2">
            <UFormField label="URL" class="sm:col-span-6"><UInput v-model="audioAddUrl" size="xs" placeholder="https://www.youtube.com/watch?v=… oder https://open.spotify.com/…" /></UFormField>
            <UFormField label="Name" class="sm:col-span-3"><UInput v-model="audioAddName" size="xs" /></UFormField>
            <UFormField label="Typ" class="sm:col-span-2">
              <USelect v-model="audioAddKind" :items="[{label:'Musik (Loop)',value:'music'},{label:'SFX',value:'sfx'}]" value-key="value" size="xs" />
            </UFormField>
            <UButton color="primary" size="xs" icon="i-lucide-plus" :disabled="!audioAddUrl || !audioAddName || !audioAddProvider" :loading="audioAdding" class="sm:col-span-1" @click="addAudioTrack">
              Add
            </UButton>
          </div>
          ...
          <div class="text-[10px] uppercase tracking-widest text-ink-300 mb-1 mt-3">Eigene Datei (MP3/OGG/WAV, max 20 MB)</div>
          <div class="grid sm:grid-cols-12 gap-2 items-end mb-2">
            <UFormField label="Audiodatei" class="sm:col-span-5">
              <input type="file" accept="audio/*" class="block w-full text-xs" @change="onAudioUploadFile">
            </UFormField>
            <UFormField label="Name" class="sm:col-span-3"><UInput v-model="audioUploadName" size="xs" /></UFormField>
            <UFormField label="Typ" class="sm:col-span-2">
              <USelect v-model="audioUploadKind" :items="[{label:'Musik (Loop)',value:'music'},{label:'SFX (1×)',value:'sfx'}]" value-key="value" size="xs" />
            </UFormField>
            <UButton color="primary" size="xs" icon="i-lucide-upload" :disabled="!audioUploadFile" :loading="audioUploading" class="sm:col-span-2" @click="uploadAudio">
              Hochladen
            </UButton>
          </div>
          ...
          <div v-if="audioMusicTracks.length" class="space-y-1">
            ...
            <div class="flex flex-wrap gap-1">
              <div v-for="t in audioMusicTracks" :key="t.id" class="flex items-center gap-1 text-xs bg-white/60 border border-parchment-700/30 rounded pl-2">
                <span class="text-[10px] uppercase font-bold"
                  :style="{ color: t.provider === 'youtube' ? '#dc2626' : t.provider === 'spotify' ? '#10b981' : '#3b82f6' }">
                  {{ t.provider === 'youtube' ? 'YT' : t.provider === 'spotify' ? 'SP' : 'UP' }}
                </span>
                <button type="button" class="flex items-center gap-1 px-1 py-1 hover:text-[var(--color-accent)]"
                  :class="audioState?.trackId === t.id && audioState?.isPlaying ? 'text-[var(--color-accent)] font-semibold' : ''"
                  @click="dmPlayMusic(t.id)">
                  <UIcon name="i-lucide-play" class="size-3" /> {{ t.name }}
                </button>
                <button type="button" class="px-1 py-1 text-ink-300 hover:text-red-700" @click="dmDeleteTrack(t.id)">
          ...
          <div v-if="audioSfxTracks.length" class="space-y-1 mt-2">
            ...
            <div class="flex flex-wrap gap-1">
              <div v-for="t in audioSfxTracks" :key="t.id" class="flex items-center gap-1 text-xs bg-white/60 border border-parchment-700/30 rounded pl-2">
                <span class="text-[10px] uppercase font-bold"
                  :style="{ color: t.provider === 'youtube' ? '#dc2626' : t.provider === 'spotify' ? '#10b981' : '#3b82f6' }">
                  {{ t.provider === 'youtube' ? 'YT' : t.provider === 'spotify' ? 'SP' : 'UP' }}
                </span>
                <button type="button" class="flex items-center gap-1 px-1 py-1 hover:text-[var(--color-accent)]" @click="dmTriggerSfx(t.id)">
                  <UIcon name="i-lucide-volume-2" class="size-3" /> {{ t.name }}
                </button>
                <button type="button" class="px-1 py-1 text-ink-300 hover:text-red-700" @click="dmDeleteTrack(t.id)">
                  <UIcon name="i-lucide-x" class="size-3" />
                </button>
          ...
          <p v-if="!audioTracks.length" class="text-xs text-ink-300 italic">Noch keine Tracks. Link einfügen oder Datei hochladen.</p>
        </div>
      </div>
    </div>

    <!-- Resize-Handle -->
    <div class="hidden lg:flex w-3 cursor-col-resize self-stretch rounded items-center justify-center group flex-none touch-none"
      ...
      <UIcon name="i-lucide-grip-vertical" class="size-4 transition-colors"
        :class="resizing ? 'text-[var(--color-accent)]' : 'text-ink-300 group-hover:text-[var(--color-accent)]'" />
    </div>

    <!-- Chat-Sidebar -->
    <aside class="parchment-card p-3 flex-col flex-none w-full lg:w-[var(--chat-w)]"
      ...
      <div class="flex items-center justify-between mb-2">
        ...
        <NuxtLink :to="`/groups/${groupId}`" class="text-xs text-ink-400 hover:text-[var(--color-accent)]">
          → Gruppe
        </NuxtLink>
      </div>
      ...
      <template #body>
        <div class="space-y-3">
          <UFormField label="Charakter">
            <USelect v-model="newToken.characterId" :items="charOptions" value-key="value" class="w-full" />
          </UFormField>
          <UFormField v-if="!newToken.characterId" label="Name">
          ...
          <UFormField label="Beschreibung (NPC-Karte, optional)">
            <UTextarea v-model="newToken.description" :rows="3" :maxlength="4000" />
          </UFormField>
    ...
    <UModal v-model:open="editingTokenId" :title="editing?.name ?? 'Token'">
      <template #body>
        <div v-if="editing" class="space-y-3">
          <div class="grid grid-cols-2 gap-3">
            <UFormField label="HP aktuell"><UInput v-model.number="editing.hp" type="number" /></UFormField>
            <UFormField label="HP max"><UInput v-model.number="editing.hpMax" type="number" /></UFormField>
          </div>
          <div>
            <div class="text-xs uppercase tracking-widest text-ink-300 mb-2">Zustände</div>
            <div class="flex flex-wrap gap-1">
              <button v-for="c in TOKEN_CONDITIONS" :key="c.id" type="button"
                class="flex items-center gap-1 px-2 py-1 rounded border-2 text-xs font-semibold transition"
                :class="isConditionActive(c.id) ? '' : 'bg-white/60 text-ink-400 border-parchment-700/40 hover:bg-white/90'"
                :style="isConditionActive(c.id) ? condStyle(c) : {}"
                :title="c.hint" @click="toggleCondition(c.id)">
                <UIcon :name="c.icon" class="size-3.5" /><span>{{ c.label }}</span>
              </button>
            </div>
          </div>
          <UFormField label="Eigene Status-Tags (Komma)">
            <UInput v-model="customStatusText" :maxlength="200" />
          </UFormField>
      ...
      <template #footer>
        <div class="flex gap-2 justify-end">
          <UButton variant="ghost" color="error" icon="i-lucide-trash-2" @click="removeToken">Entfernen</UButton>
      ...
      <template #body>
        <div v-if="infoToken" class="space-y-3">
          <div v-if="tokenImageSrc(infoToken)" class="flex justify-center">
            <img :src="tokenImageSrc(infoToken) ?? ''" :alt="infoToken.name" class="max-h-80 max-w-full rounded-lg shadow-lg">
          </div>
          <div class="text-sm">
            ...
            <div v-if="tokenConditions(infoToken).length || tokenCustomLabels(infoToken).length" class="mb-3">
              <div class="text-[10px] uppercase text-ink-300 mb-1">Zustände</div>
              <div class="flex flex-wrap gap-1">
                <span v-for="c in tokenConditions(infoToken)" :key="c.id"
                  class="flex items-center gap-1 px-2 py-1 rounded border-2 text-xs font-semibold"
                  :style="condStyle(c)">
                  <UIcon :name="c.icon" class="size-3.5" /> {{ c.label }}
                </span>
                <span v-for="(lab, i) in tokenCustomLabels(infoToken)" :key="`custom-${i}`"
          ...
          <div v-if="infoToken.description" class="whitespace-pre-wrap text-sm leading-relaxed">{{ infoToken.description }}</div>
          <div v-else class="text-sm italic text-ink-300">Keine Beschreibung hinterlegt.</div>
        </div>
      </template>
      <template #footer>
        <div class="flex gap-2 justify-end">
          <UButton v-if="infoToken && canMoveToken(infoToken)" variant="outline" icon="i-lucide-edit"
